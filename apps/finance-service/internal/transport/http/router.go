...
func NewRouter(financeHandler *handler.FinanceHandler, coaHandler *handler.CoaHandler, postingHandler *handler.PostingHandler, cashHandler *handler.CashHandler, postingCaseHandler *handler.PostingCaseHandler, reportingHandler *handler.ReportingHandler, counterpartyHandler *handler.CounterpartyHandler) http.Handler {
	...
	// ── Reporting foundation (P3a) ──
	// COB job: platform EOD engine posts here in-cluster (no gateway policy).
	mux.HandleFunc("/internal/jobs/trial-balance-daily", reportingHandler.RunTrialBalanceDailyJob)
	mux.HandleFunc("/api/finance/trial-balance/daily", method("GET", reportingHandler.TrialBalanceDaily))
	mux.HandleFunc("/api/finance/statements", method("GET", reportingHandler.ListStatements))
	...
	mux.HandleFunc("/api/finance/journal-entries/by-case/{case_id}", method("GET", postingHandler.GetJournalEntryByCase))
	mux.HandleFunc("/api/finance/journal-entries/{entry_no}", method("GET", postingHandler.GetJournalEntry))
	mux.HandleFunc("/api/finance/ledger", method("GET", postingHandler.GetLedger))
	...
	mux.HandleFunc("/api/finance/posting-cases/import", method("POST", postingCaseHandler.ImportPostingCases))
