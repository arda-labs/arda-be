...
func TestInternalAIService_RequiresAIServiceToken(t *testing.T) {
	...
	valid, err := identity.Issue(secret, "ai-service", "finance-service", time.Now(), time.Minute)
	if err != nil {
	...
	tests := []struct {
		...
		{name: "wrong source", token: func() string {
			t, _ := identity.Issue(secret, "workflow-service", "finance-service", time.Now(), time.Minute)
			return t
		}(), status: http.StatusForbidden},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := httptest.NewRequest(http.MethodGet, "/internal/ai/accounts/a1", nil)
			if tt.token != "" {
