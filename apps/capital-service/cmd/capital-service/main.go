...
func main() {
	...
	internalReportingHandler := handler.NewInternalReportingHandler(capitalSvc)
	...
	srv := &http.Server{
		Addr:         cfg.HTTPAddr,
		Handler:      ardahttp.MetricsMiddleware(cfg.AppName, transport.NewRouter(capitalHandler, internalAIHandler, internalReportingHandler)),
