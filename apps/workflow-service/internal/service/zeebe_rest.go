...
func (c *ZeebeRestClient) ResolveIncident(ctx context.Context, incidentKey int64) error {
	...
	httpReq, err := http.NewRequestWithContext(ctx, http.MethodPost,
		fmt.Sprintf("%s/v2/incidents/%d/resolution", c.baseURL, incidentKey), nil)
	if err != nil {
		return err
	}
	httpReq.Header.Set("Content-Type", "application/json")
	...
	if resp.StatusCode >= 300 {
		b, _ := io.ReadAll(resp.Body)
		return fmt.Errorf("resolve incident HTTP %d: %s", resp.StatusCode, strings.TrimSpace(string(b)))
	}
	return nil
}
