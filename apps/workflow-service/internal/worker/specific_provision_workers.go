...
func (w *SpecificProvisionWorkers) execute() worker.JobHandler {
	return func(client worker.JobClient, job entities.Job) {
		...
		if err := w.loanClient.ResolveSpecificProvision(crmJobContext(job), id, "APPROVE", decidedBy, note, dataVersionFromVars(vars)); err != nil {
...
func (w *SpecificProvisionWorkers) cancel() worker.JobHandler {
	return func(client worker.JobClient, job entities.Job) {
		...
		if err := w.loanClient.ResolveSpecificProvision(crmJobContext(job), id, "REJECT", decidedBy, note, dataVersionFromVars(vars)); err != nil {
			if status.Code(err) == codes.Aborted {
				w.failJobTerminal(client, job, "Loan Conflict: "+status.Convert(err).Message())
				return
			}
...
func (w *SpecificProvisionWorkers) failJobTerminal(client worker.JobClient, job entities.Job, reason string) {
	slog.Warn("workflow specific provision job terminal", "jobType", job.GetType(), "reason", reason)
