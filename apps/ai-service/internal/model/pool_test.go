...
func TestClientPool_ReusesSameClientForSameConfig(t *testing.T) {
	...
	client1 := pool.GetClient("tenant-1", "openai", "https://api.openai.com/v1", "key-1", "gpt-4o")
	client2 := pool.GetClient("tenant-1", "openai", "https://api.openai.com/v1", "key-1", "gpt-4o")
	...
	client3 := pool.GetClient("tenant-1", "openai", "https://api.openai.com/v1", "key-2-new", "gpt-4o")
	if client1 == client3 {
	...
	client4 := pool.GetClient("tenant-1", "openai", "https://api.openai.com/v1", "key-2-new", "gpt-4o")
	if client3 == client4 {
...
func TestClientPool_MultiTenantIsolation(t *testing.T) {
	...
	clientA := pool.GetClient("tenant-A", "openai", "https://api.openai.com/v1", "key-A", "gpt-4o")
	clientB := pool.GetClient("tenant-B", "openai", "https://api.openai.com/v1", "key-B", "gpt-4o")
...
func TestClientPool_ConfigurationChangeResetsCircuitGuard(t *testing.T) {
	pool := NewClientPool(nil)
	first := pool.GetProvider("tenant-1", "openai-compatible", "https://one.example/v1", "key-1", "model-a")
	if len(pool.guards) != 1 {
		t.Fatalf("expected one circuit guard, got %d", len(pool.guards))
	}
	second := pool.GetProvider("tenant-1", "openai-compatible", "https://two.example/v1", "key-2", "model-b")
	if first == second {
