...
func TestInternalAIService_RequiresAIServiceToken(t *testing.T) {
	...
	valid, err := identity.Issue(secret, "ai-service", "crm-service", time.Now(), time.Minute)
	if err != nil {
	...
	tests := []struct {
		...
		{name: "wrong source", token: func() string {
			t, _ := identity.Issue(secret, "workflow-service", "crm-service", time.Now(), time.Minute)
			return t
		}(), status: http.StatusForbidden},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := httptest.NewRequest(http.MethodGet, "/internal/ai/customers/u1", nil)
			if tt.token != "" {
