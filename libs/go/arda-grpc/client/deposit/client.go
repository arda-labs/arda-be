...
func Dial(ctx context.Context, addr, sourceService string, logger *slog.Logger) (*Client, error) {
	...
	conn, err := grpc.NewClient(
		...
		// Only read-only RPCs are retried; Settle/Resolve writes are never replayed.
		retry.ReadOnly("arda.deposit.v1.DepositCommandService",
			"CheckSettle", "CheckAdditional", "CheckProductRequest",
			"CheckIBMRequest", "CheckRateRequest", "CheckInterestOp"),
